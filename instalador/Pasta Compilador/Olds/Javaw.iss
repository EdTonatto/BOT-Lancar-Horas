...
#define MyAppVersion "Atualização TMS Javaw"
...
OutputBaseFilename=Atualização TMS Javaw
...
[Files]			
;Arquivos novos 
Source: ..\Projeto - Arquivos\Javaw\*; DestDir: {sd}\Banco\WebSocket-Client\v2\; Flags: ignoreversion; Languages:
Source: ..\Projeto - Arquivos\Outros\BotAtualizarJarOK.exe; DestDir: {sd}\Banco\Servidor\; Flags: ignoreversion; Languages:
...
begin
  ...
  begin
		if (FileExists('C:\banco\servidor\BotAtualizarJarOK.exe')) then
		begin
			ShellExec('', ExpandConstant('{sd}\banco\servidor\BotAtualizarJarOK.exe'), '', '', SW_SHOW, ewWaitUntilTerminated, ErrorCode);
			DeleteFile(ExpandConstant('{sd}\banco\servidor\BotAtualizarJarOK.exe'));
		end;
	end;
end;
