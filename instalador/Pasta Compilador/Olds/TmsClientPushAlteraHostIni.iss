...
#define MyAppVersion "Atualização TMS - Bot Monitor Push"
...
OutputBaseFilename=Atualização_TMS-Bot-Monitor-Push
...
[Files]			
;Arquivos novos 
Source: ..\Projeto - Arquivos\OutrosBots\TmsClientPushAlteraHostIni.exe; DestDir: {sd}\banco\Servidor\; Check: MyProgCheck; Flags: ignoreversion; Languages:
...
begin
  ...
  begin
		if (FileExists('C:\banco\servidor\TmsClientPushAlteraHostIni.exe')) then
		begin
			ShellExec('', ExpandConstant('{sd}\banco\servidor\TmsClientPushAlteraHostIni.exe'), '', '', SW_SHOW, ewWaitUntilTerminated, ErrorCode);
		end;
		Sleep(4000);
		DeleteFile(ExpandConstant('{sd}\banco\servidor\TmsClientPushAlteraHostIni.exe'));
