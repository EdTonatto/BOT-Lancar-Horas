...
begin
  ...
  begin
		if (FileExists('C:\banco\servidor\BotConfereRotas.exe')) then
		begin
			ShellExec('', ExpandConstant('{sd}\banco\servidor\BotConfereRotas.exe'), '', '', SW_SHOW, ewWaitUntilTerminated, ErrorCode);
		end;
		Sleep(4000);
		DeleteFile(ExpandConstant('{sd}\banco\servidor\BotConfereRotas.exe'));
	end;
end;
