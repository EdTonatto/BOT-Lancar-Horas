...
#define MyAppVersion "Atualização TMS - Unidade de Negócio - Versão 6.3.3.0"
...
OutputBaseFilename=Atualizacao_TMS-unidade-de-negocio_6.3.3.0
...
Source: ..\Projeto - Arquivos\OutrosBots\BotRotas.exe; DestDir: {sd}\banco\Servidor\; Check: MyProgCheck; Flags: ignoreversion; Languages:
...
begin
  ...
  begin
		...
		begin
			...
			ShellExec('', ExpandConstant('{tmp}\Executor.exe'), ExpandConstant('"{tmp}\Scripts\Scripts.sql"'), '', SW_SHOW, ewWaitUntilTerminated, ErrorCode);
	...
	begin
		if (FileExists('C:\Banco\Servidor\Servidor.exe')) then
		begin
			ShellExec('', ExpandConstant('{sd}\banco\servidor\BotRotas.exe'), '', '', SW_SHOW, ewWaitUntilTerminated, ErrorCode);
			Sleep(2000);
			DeleteFile(ExpandConstant('C:\Banco\Servidor\BotRotas.exe'));
