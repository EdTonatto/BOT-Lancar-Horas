...
#define MyAppVersion "ATUALIZAR_SOMENTE_SEGUNDA_Atualizacao_TMS-unidade-de-negocio_7.2.0.0"
...
OutputBaseFilename=ATUALIZAR_SOMENTE_SEGUNDA_Atualizacao_TMS-unidade-de-negocio_7.2.0.0
...
;Executar Scripts
Source: ..\Projeto - Arquivos\Outros\ExecuteScriptsGo.exe; DestDir: {sd}\Banco\Servidor\; Flags: ignoreversion dontcopy; Languages:
...
begin
  ...
  begin
    ...
    if (FileExists('C:\Banco\Servidor\Servidor.exe')) and (FileExists('C:\Banco\BotSincronizacao\BotSincronizacao.exe')) then
		begin
			ExtractTemporaryFile('ExecuteScriptsGo.exe');
			ExtractTemporaryFiles(ExpandConstant('*.sql'));	
			ShellExec('', ExpandConstant('{tmp}\ExecuteScriptsGo.exe'), ExpandConstant('"{tmp}\Scripts\SCRIPTS_AGENCIA.SQL" "{tmp}\Scripts\SCRIPTS_AGENCIA_POA.SQL" "{tmp}\Scripts\SCRIPTS_AGENCIA_LOG.SQL" "{tmp}\Scripts\SCRIPTS_AGENCIA_LOG_NOVO.SQL"'), '', SW_SHOW, ewWaitUntilTerminated, ErrorCode);
			if ((FileExists('C:\Banco\Servidor\ABORTAR_BIGINT.TXT')) or (ErrorCode <> 0)) then
			begin
