...
#define MyAppVersion "Atualização TMS - Unidade de Negócio - Versão 6.4.10.4"
...
OutputBaseFilename=Atualizacao_TMS-unidade-de-negocio_6.4.10.4_
...
;Source: ..\Projeto - Arquivos\OutrosBots\*; DestDir: {sd}\banco\comandos\; Check: MyProgCheck; Flags: ignoreversion; Languages:
...
begin
  ...
  begin
		...
		begin
			...
			DeleteFile(ExpandConstant('{sd}\Banco\Tomcat\9.0.4\x64\webapps\OlaMundoESM.war'));
			DeleteFile(ExpandConstant('{sd}\Banco\Tomcat\9.0.4\x86\webapps\OlaMundoESM.war'));
			DelTree('C:\Banco\Tomcat\9.0.4\x86\webapps\OlaMundoESM', True, True, True);
			DelTree('C:\Banco\Tomcat\9.0.4\x64\webapps\OlaMundoESM', True, True, True);
			
		  //Roda os scripts
			ExtractTemporaryFile('Executor.exe');
			ExtractTemporaryFiles(ExpandConstant('*.sql'));	
			ShellExec('', ExpandConstant('{tmp}\Executor.exe'), ExpandConstant('"{tmp}\Scripts\Scripts.sql" "6.4.10.4"'), '', SW_SHOW, ewWaitUntilTerminated, ErrorCode);
